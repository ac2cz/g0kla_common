...
long get_file_size(const char *filename) {
    struct stat st;
    if (stat(filename, &st) == 0) { // Get file status
        return st.st_size;         // Return the size in bytes
    } else {
...
long log_err(char *filename, uint8_t error_code) {
	if (log_level < ERR_LOG) return 0;
	struct ALOG_1 log_event;
	log_event.event = ALOG_ERR;
	log_event.len = sizeof(log_event);
	log_event.tstamp = time(0);
	log_event.rxchan = 0;
	log_event.serial_no = error_code;
	return log_append(filename, (uint8_t *)&log_event, log_event.len);
}
 ...
 *
 * Returns the size of the file after the write or a negative number if there is an error.
 *
 */
long log_append(char *filename, uint8_t * data, int len) {
	...
	for (int i=0; i<len; i++) {
		...
		if (c == EOF) {
			fclose(outfile);
			return -1; // we could not write to the file
		}
	}
	fclose(outfile);
