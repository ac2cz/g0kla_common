...
/* Global vars defined in common_config.h and declared here */
int g_common_frames_queued = 0;
...
char registered_callsign[MAX_CALLSIGN_LEN];
...
int tnc_get_frames_queued() {
	return g_common_frames_queued;
}

int tnc_busy() {
	if (tnc_get_frames_queued() > g_common_max_frames_in_tx_buffer) return true;
	return false;
}
/**
 ...
 * Must call this before we start the listen process
 * Returns 0 if successful otherwise 1
 */
int tnc_register_callsign(char *callsign) {
	strlcpy( registered_callsign, callsign, sizeof(registered_callsign) );
	return EXIT_SUCCESS;
}

int register_callsign(char *callsign) {
	struct t_agw_header header;
	memset (&header, 0, sizeof(header));
...
/**
 * Un-Registers the callsign of this station with Direwolf using an AGW x type frame
 * Returns 0 if successful otherwise 1
 */
int tnc_unregister_callsign(char *callsign) {
	struct t_agw_header header;
	memset (&header, 0, sizeof(header));
	header.data_kind = 'x'; // register callsign
	strlcpy( header.call_from, callsign, sizeof(header.call_from) );
...
void *tnc_listen_process(void * arg) {
	...
	while (listen_thread_called) {
		...
		while (rc == EXIT_FAILURE) {
			...
			if (rc == EXIT_SUCCESS) {
				...
				rc = register_callsign(registered_callsign);
					if (rc != EXIT_SUCCESS) {
						error_print("\n Error : Could not monitor register callsign\n");
					}
				listen_thread_called = true;
				debug_print ("Connected to tnc: %s\n", name);
			} else {
				//error_print ("Could not open TCP connect to tnc, retry %d of %d\n",retries, max_retries);
				sleep(1);
			}
		}
	...
	debug_print("Exiting TNC Thread: %s\n", name);
	if (sockfd != -1)
		close(sockfd);
	sockfd = -1;
